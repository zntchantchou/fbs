...
function UploaderItem({ index, filename, url, moveCard, onDelete }: Props) {
  ...
  return (
    <div
      key={index}
      ref={dragItemRef}
      className="mb-4 cursor-move border-gray-300 bg-gray-200 hover:bg-gray-300 border-2 rounded-md flex justify-center h-20 md:max-w-2xl relative"
    >
      <div
        className="absolute top-1 right-1 rounded-full h-10 w-10 flex items-center justify-center"
        aria-label="delete the image"
        onClick={() => onDelete(index)}
      >
        {/* DELETE IMAGE ICON */}
        <X className="hover:text-gray-400 cursor-pointer" height={14} />
      </div>
      <div className=" h-full flex items-center bold text-white p-1 w-16">
        <div className="h-[2.3rem] w-[2.3rem] rounded-full bg-slate-400 text-bold text-lg flex items-center justify-center">
          {index + 1}
        </div>
      </div>{" "}
      <div className="w-3/4 flex justify-center ">
        ...
        />
        <div className="h-full w-full flex pl-4 justify-end items-center text-xs sm:text-sm lg:text-base">
          <span className="font-bold mr-2">file: </span>
          <p className="max-w-2xl text-gray-600">
            {filename.slice(0, 23)} {filename.length > 23 && "..."}
          </p>
        </div>
      </div>
    </div>
